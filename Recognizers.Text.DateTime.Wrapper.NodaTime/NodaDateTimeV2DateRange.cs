...
{
    /// <summary>
    /// A DateTime GenericRange with both a Start and End LocalDate that contain the start LocalDate and end LocalDate recognized. 
    /// </summary>
    public class NodaDateTimeV2DateRange : DateTimeV2ObjectWithValue<DateTimeV2Range<LocalDate>>
    {
        ...
        protected override void InitializeValue(IDictionary<string, string> value)
        {
            ...
            {
                throw new ArgumentException($"Failed to parse the start value \"{value["start"]}\" with the expected format \"uuuu-MM-dd\"", startDateParsed.Exception);
            }
            ...
            {
                throw new ArgumentException($"Failed to parse the end value \"{value["end"]}\" with the expected format \"uuuu-MM-dd\"", endDateParsed.Exception);
            }


            this.Value = new DateTimeV2Range<LocalDate>(startDate, endDate);
        }
    }
}
